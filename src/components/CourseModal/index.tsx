            ...
            <ScrollArea className="flex-grow mt-6 pr-4 -mr-4">
                <ul className="space-y-2">
                    {course.toc.map((item, index) => (
                        <li key={index} className="bg-secondary/50 border-l-4 border-primary/50 rounded-r-lg p-4 transition-all hover:bg-secondary">
                           <div className="flex justify-between items-center">
                                <h5 className="font-headline text-xl font-semibold text-white tracking-wider">{item.title}</h5>
                                <Badge variant="outline" className="font-body text-sm border-primary/30 text-primary">{item.time}</Badge>
                           </div>
                           <p className="font-body text-base text-muted-foreground mt-2">{item.description}</p>
                        </li>
                    ))}
                </ul>
            </ScrollArea>
